...
namespace SharpCircuit
{
    public class Circuit
    {
        ...
        private int currentId = -1; //ADDED BY TIM

        private int nodePrevId //ADDED BY TIM
        {
            get
            {
        ...
        public void AddElement(ICircuitElement elm)
        {
            if (!elements.Contains(elm))
            {
                ...
                for (int x = 0; x < elm.getLeadCount(); x++)
                    nodeMesh[nodeMesh.Count - 1][x] = nodePrevId; //ADDED BY TIM [nodePrevId]
        ...
        public void Connect(int leftNdx, int leftLeadNdx, int rightNdx, int rightLeadNdx)
        {
            ...
            bool empty = leftConn < -1 && rightConn < -1;//ADDED BY TIM [condition rules "==" to "<"]
            if (empty)
            {
            ...
            if (leftConn < -1)//ADDED BY TIM [condition rules "==" to "<"]
        ...
        public void analyze()
        {
            ...
            if (!gotGround && !gotRail && voltageElm != null)
            {
                ...
                if (nodeMesh[elm_ndx][0] < -1) //ADDED BY TIM [generating id for the voltage elm's first terminal]
                    nodeMesh[elm_ndx][0] = snowflake.NextId();
                pushNode(nodeMesh[elm_ndx][0], false);
            }
            else
            {
            ...
            for (int i = 0; i != elements.Count; i++)
            {
                ...
                for (int leadX = 0; leadX != leads; leadX++)
                {
                    ...
                    else
                    {
						...
						if(node_ndx == 0) //ADDED BY TIM [from "leadNode" to "node_ndx"]
                        {
            ...
            for (int i = 0; i < elements.Count; i++) //ADDED BY TIM [Old check version commented] Checking leads that are not connected(nodeId not generated by snowflake) to any other elements and also checking each single elem's leads connection for internal current existance. 
            {
                ICircuitElement ce = elements[i];
                long[] leads = nodeMesh[i];
                ...
                for (int j = 0; j < ce.getLeadCount(); j++)
                {
                    if (leads[j] < -1)
                    {
                        ...
                        var ndx = nodeList.IndexOf(leads[j]);
                        leads[j] = id;
                        nodeList[ndx] = id;
                        stampResistor(0, ndx, 1E8);
                    }
                    
                    for (int k = 0; k < ce.getLeadCount(); k++)
                    {
                        if (j == k) continue;
                        if (!ce.leadsAreConnected(j, k))
                        {
                            stampResistor(ce.getLeadNode(j), ce.getLeadNode(k), 1E8);
                        }
                    }
                }
            }
